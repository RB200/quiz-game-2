...
def clientthread(con,add):
    ...
    while True:
        try:
            ...
            print(message)
            if message:
                print(message.split(': ')[1].lower() == answer)
                if str(message.split(': ')[1]) == str(answer): 
                    score += 1
                    con.send(f'Bravo! Your score is {score}\n'.encode('utf-8'))
                else:
                    print((len(message.split(': ')[1]), message.split(': ')[1]), len(answer))
                    con.send('Incorrect answer! The right answer was {}. Better luck next time.\n'.format(answer).encode('utf-8'))
                remove_question(idx)
                idx, question, answer = get_random_question_answer(con)
                print(message)
            else:
...
while True:
    ...
    print(message)
    thread = Thread(target=clientthread,args=(con,name))
    thread.start()
